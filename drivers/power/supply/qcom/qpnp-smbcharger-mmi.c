			...
			u16 addr, int count)
{
	...
	if (addr == 0) {
		SMB_ERR(chip, "addr cannot be zero addr=0x%02x sid=0x%02x rc=%d\n",
...
static int get_prop_batt_resistance_id(struct smbchg_chip *chip)
{
	...
	rc = get_property_from_fg(chip, POWER_SUPPLY_PROP_RESISTANCE_ID,
			&rbatt);
	if (rc) {
		SMB_DBG(chip, "Couldn't get resistance id rc = %d\n", rc);
...
static int get_prop_batt_full_charge(struct smbchg_chip *chip)
{
	...
	if (rc) {
		SMB_DBG(chip, "Couldn't get charge_full rc = %d\n", rc);
...
static void update_typec_status(struct smbchg_chip *chip)
{
	...
	if (type.intval != POWER_SUPPLY_TYPE_UNKNOWN) {
		...
		SMB_ERR(chip, "SMB Type-C mode = %d, current=%d\n",
								...
								int period_s)
{
	...
	if (i < 0) {
		SMB_ERR(chip, "Cannot find %ds in aicl rerun period\n",
				period_s);
		return -EINVAL;
	}
	...
	SMB_DBG(chip, "aicl rerun period set to %ds\n",
...
static int smbchg_request_dpdm(struct smbchg_chip *chip, bool enable)
{
	...
	if (!chip->dpdm_reg && of_get_property(chip->dev->of_node,
		...
		if (IS_ERR(chip->dpdm_reg)) {
			...
			SMB_ERR(chip, "Couldn't get dpdm regulator rc=%d\n",
					rc);
			chip->dpdm_reg = NULL;
			return rc;
		}
	...
	if (enable) {
		if (!regulator_is_enabled(chip->dpdm_reg)) {
			SMB_DBG(chip, "enabling DPDM regulator\n");
			rc = regulator_enable(chip->dpdm_reg);
			if (rc < 0)
				SMB_ERR(chip, "Couldn't enable dpdm regulator rc=%d\n",
					rc);
		}
	} else {
		if (regulator_is_enabled(chip->dpdm_reg)) {
			SMB_DBG(chip, "disabling DPDM regulator\n");
			rc = regulator_disable(chip->dpdm_reg);
			if (rc < 0)
				SMB_ERR(chip, "Couldn't disable dpdm regulator rc=%d\n",
					rc);
		}
	}
...
static void smbchg_usb_update_online_work(struct work_struct *work)
{
	...
	if (chip->usb_online != online) {
		SMB_DBG(chip, "setting usb psy online = %d\n", online);
							...
							int current_ma)
{
	...
	if (current_ma == CURRENT_100_MA) {
		...
		if (rc < 0) {
			SMB_ERR(chip, "Couldn't set CFG_USB_2 rc=%d\n", rc);
			return rc;
		}
		...
		if (rc < 0) {
			SMB_ERR(chip, "Couldn't set ICL_OVERRIDE rc=%d\n", rc);
			return rc;
		}
							...
							int current_ma)
{
	...
	case POWER_SUPPLY_TYPE_USB:
		...
		if (current_ma < CURRENT_150_MA) {
			...
			rc = smbchg_sec_masked_write(chip,
					...
					CFG_USB_2_3_SEL_BIT, CFG_USB_2);
			if (rc < 0) {
				SMB_ERR(chip, "Couldn't set CHGPTH_CFG rc = %d\n", rc);
				goto out;
			}
			rc = smbchg_masked_write(chip,
					...
					USBIN_LIMITED_MODE | USB51_100MA);
			if (rc < 0) {
				SMB_ERR(chip, "Couldn't set CMD_IL rc = %d\n", rc);
				goto out;
			}
	...
	case POWER_SUPPLY_TYPE_USB_CDP:
		if (current_ma < CURRENT_1500_MA) {
			...
			if (rc < 0)
				SMB_ERR(chip, "Couldn't set override rc = %d\n", rc);
		}
		/* fall through */
	default:
...
out:
	SMB_DBG(chip, "usb type = %d current set to %d mA\n",
...
static void smbchg_detect_parallel_charger(struct smbchg_chip *chip)
{
	...
	if (parallel_psy) {
		...
		if (rc)
			SMB_DBG(chip,"parallel-charger absent rc=%d\n", rc);
	}
}
...
static int smbchg_sw_esr_pulse_en(struct smbchg_chip *chip, bool en)
{
	...
	if (rc < 0) {
		SMB_ERR(chip,"Couldn't Vote FCC en = %d rc = %d\n", en, rc);
		return rc;
	}
...
static void smbchg_parallel_usb_disable(struct smbchg_chip *chip)
{
	...
	if (fcc_ma < 0)
		SMB_ERR(chip, "no voters for fcc, skip it\n");
	else
		smbchg_set_fastchg_current_raw(chip, fcc_ma);

	if (usb_icl_ma < 0)
		SMB_ERR(chip, "no voters for usb_icl, skip it\n");
	else
...
try_again:
	...
	SMB_DBG(chip, "reducing FCC of parallel charger to %d\n",
		pval.intval);
	/* Change it to uA */
	pval.intval *= 1000;
		...
		int total_current_ma)
{
	...
	SMB_DBG(chip, "Requested FCC = %d from parallel, got %d\n",
		...
		int *ret_total_current_ma)
{
	...
	if (fcc_ma < 0) {
		SMB_ERR(chip, "no voters for fcc! Can't enable parallel\n");
		return false;
	}
	if (usb_icl_ma < 0) {
		SMB_ERR(chip, "no voters for usb_icl, Can't enable parallel\n");
		return false;
	}
	...
	if (chip->parallel.current_max_ma == 0
		...
		SMB_DBG(chip, "Only been %lld since disable, skipping\n",
				ktime_to_ms(kt_since_last_disable));
		return false;
	}
	...
	if ((strcmp(fcc_voter, ESR_PULSE_FCC_VOTER) == 0)
		...
		SMB_DBG(chip, "FCC %d lower than %d\n",
			fcc_ma,
	...
	if (chip->parallel.initial_aicl_ma == 0) {
		if (current_limit_ma < min_current_thr_ma) {
			SMB_DBG(chip, "Initial AICL very low: %d < %d\n",
						...
						const char *client)
{
	...
	SMB_ERR(chip, "smbchg_hvdcp_enable_cb HVDCP %s\n",
			enable ? "enabled" : "disabled");
	rc = smbchg_sec_masked_write(chip,
				chip->usb_chgpth_base + CHGPTH_CFG,
				HVDCP_EN_BIT, enable ? HVDCP_EN_BIT : 0);
	if (rc < 0)
		SMB_ERR(chip, "Couldn't %s HVDCP rc=%d\n",
						...
						const char *client)
{
	...
	if (chip->parallel.current_max_ma == 0) {
		...
		if (rc < 0) {
			SMB_ERR(chip, "Can't set FCC fcc_ma=%d rc=%d\n", fcc_ma, rc);
			return rc;
		}
	}
	/*
							...
							int current_ma)
{
	...
	SMB_DBG(chip, "User setting FCC to %d\n", current_ma);

	rc = vote(chip->fcc_votable, BATT_TYPE_FCC_VOTER, true, current_ma);
	if (rc < 0)
		SMB_ERR(chip, "Couldn't vote en rc %d\n", rc);
	return rc;
}
...
static int smbchg_dcin_ilim_config(struct smbchg_chip *chip, int offset, int ma)
{
	...
	rc = smbchg_sec_masked_write(chip, chip->bat_if_base + offset,
			ZIN_ICL_MASK, i);
	if (rc)
		SMB_ERR(chip, "Couldn't write bat if offset %d value = %d rc = %d\n",
				offset, i, rc);
	return rc;
}
						...
						struct ilim_entry *ilim)
{
	...
	if (chip->current_ilim.icl_pt_ma != ilim->icl_pt_ma) {
		...
		if (rc)
			SMB_ERR(chip, "failed to write batif offset %d %dma rc = %d\n",
						...
						int dcin_uv, bool div2)
{
	...
	if (rc) {
		SMB_ERR(chip, "failed to config ilim rc = %d, dcin_uv = %d , div2 = %d, ilim = (%duV %duV %dmA %dmA %dmA)\n",
			rc, dcin_uv, div2,
			ilim->vmin_uv, ilim->vmax_uv,
			ilim->icl_pt_ma, ilim->icl_lv_ma, ilim->icl_hv_ma);
		return rc;
	...
	if (rc) {
		SMB_ERR(chip, "failed to config btm rc = %d, dcin_uv = %d , div2 = %d, ilim = (%duV %duV %dmA %dmA %dmA)\n",
			rc, dcin_uv, div2,
			ilim->vmin_uv, ilim->vmax_uv,
			ilim->icl_pt_ma, ilim->icl_lv_ma, ilim->icl_hv_ma);
		return rc;
								...
								int lvl_sel)
{
	...
	if (lvl_sel >= chip->thermal_levels) {
		SMB_ERR(chip, "Unsupported level selected %d forcing %d\n",
	...
	if (chip->therm_lvl_sel == 0) {
		...
		if (rc < 0)
			SMB_ERR(chip, "Couldn't disable USB thermal ICL vote rc=%d\n",
				rc);

		rc = vote(chip->dc_icl_votable, THERMAL_ICL_VOTER, false, 0);
		if (rc < 0)
			SMB_ERR(chip, "Couldn't disable DC thermal ICL vote rc=%d\n",
				rc);
	} else {
		thermal_icl_ma =
		...
		rc = vote(chip->dc_icl_votable, THERMAL_ICL_VOTER, true,
					thermal_icl_ma);
		if (rc < 0)
			SMB_ERR(chip, "Couldn't vote for DC thermal ICL rc=%d\n", rc);
	}
...
static int smbchg_float_voltage_set(struct smbchg_chip *chip, int vfloat_mv)
{
	...
	if (parallel_psy) {
		...
		if (rc)
			SMB_ERR(chip, "Couldn't set float voltage on parallel psy rc: %d\n",
				rc);
	}
				...
				bool flash_active)
{
	...
	if (chip->flash_active == flash_active) {
		SMB_DBG(chip, "Fsw not changed, flash_active: %d\n",
			flash_active);
		return 0;
	}
	...
	if (rc < 0) {
		SMB_ERR(chip, "Couldn't set switching frequency multiplier rc=%d\n",
				rc);
		return rc;
	}
	...
	if (rc < 0) {
		SMB_ERR(chip, "Cannot set switching freq: %d\n", rc);
		return rc;
	}

	SMB_DBG(chip, "Fsw @ %sHz\n", flash_active ? "1M" : "750K");
						...
						const char *client)
{
	...
	if (rc < 0) {
		SMB_ERR(chip, "Couldn't vote for hw rerun rc= %d\n", rc);
		return rc;
	}
						...
						const char *client)
{
	...
	if (rc < 0)
		SMB_ERR(chip, "Couldn't write to MISC_TRIM_OPTIONS_15_8 rc=%d\n", rc);
...
static void smbchg_aicl_deglitch_wa_en(struct smbchg_chip *chip, bool en)
{
	...
	if (rc < 0) {
		SMB_ERR(chip, "Couldn't vote %s deglitch rc=%d\n",
				en ? "short" : "long", rc);
		return;
	}
	SMB_DBG(chip, "AICL deglitch set to %s\n", en ? "short" : "long");

	rc = vote(chip->hw_aicl_rerun_enable_indirect_votable,
			VARB_WORKAROUND_VOTER, en, 0);
	if (rc < 0) {
		SMB_ERR(chip, "Couldn't vote hw aicl rerun rc= %d\n", rc);
		return;
	}
...
static int smbchg_config_chg_battery_type(struct smbchg_chip *chip)
{
	...
	if (rc) {
		SMB_DBG(chip, "Unable to read battery-type rc=%d\n", rc);
		return 0;
	}
	if (!strcmp(prop.strval, UNKNOWN_BATT_TYPE) ||
		!strcmp(prop.strval, LOADING_BATT_TYPE)) {
		SMB_DBG(chip, "Battery-type not identified\n");
		return 0;
	}
	...
	if (!batt_node) {
		SMB_DBG(chip, "No batterydata available\n");
		return 0;
	}
	...
	if (rc < 0) {
		SMB_DBG(chip, "Unable to read battery-id rc=%d\n", rc);
		return 0;
	}
	...
	if (IS_ERR_OR_NULL(profile_node)) {
		...
		SMB_ERR(chip, "couldn't find profile handle %d\n", rc);
		return rc;
	}
	...
	rc = of_property_read_u32(profile_node, "qcom,max-voltage-uv",
						&max_voltage_uv);
	if (rc) {
		SMB_WARN(chip, "couldn't find battery max voltage rc=%d\n", rc);
		ret = rc;
	} else {
		if (chip->vfloat_mv != (max_voltage_uv / 1000)) {
			SMB_INFO(chip,"Vfloat changed from %dmV to %dmV for battery-type %s\n",
	...
	rc = of_property_read_u32(profile_node, "qcom,chg-term-ua",
						&iterm_ua);
	if (rc && rc != -EINVAL) {
		SMB_WARN(chip, "couldn't read battery term current=%d\n", rc);
		ret = rc;
	} else if (!rc) {
		if (chip->iterm_ma != (iterm_ua / 1000)
				&& !chip->iterm_disabled) {
			SMB_INFO(chip,"Term current changed from %dmA to %dmA for battery-type %s\n",
...
static int smbchg_external_otg_regulator_disable(struct regulator_dev *rdev)
{
	...
	if (rc < 0) {
		SMB_ERR(chip, "Couldn't enable HVDCP rc=%d\n", rc);
		return rc;
	}
		...
		enum led_brightness value)
{
	...
	if (!is_bms_psy_present(chip)) {
		SMB_ERR(chip, "Couldn't access bms psy\n");
		return;
	}
	...
	if (rc)
		SMB_ERR(chip, "Couldn't write CHG_LED rc=%d\n",
				rc);
}
...
static int smbchg_register_chg_led(struct smbchg_chip *chip)
{
	...
	if (rc) {
		SMB_ERR(chip, "unable to register charger led, rc=%d\n",
				rc);
		return rc;
	}
						...
						enum power_supply_type type)
{
	...
	SMB_DBG(chip, "Type %d: setting mA = %d\n",
		type, current_limit_ma);
	rc = vote(chip->usb_icl_votable, PSY_ICL_VOTER, true,
				current_limit_ma);
	if (rc < 0) {
		SMB_ERR(chip, "Couldn't vote for new USB ICL rc=%d\n", rc);
		goto out;
	}
	...
	if (rc < 0)
		SMB_ERR(chip, "Couldn't set charger optimal mode rc=%d\n", rc);
...
static int force_9v_hvdcp(struct smbchg_chip *chip)
{
	...
	if (rc) {
		SMB_ERR(chip, "Couldn't set hvdcp config in chgpath_chg rc=%d\n", rc);
		return rc;
	}
	...
	rc |= smbchg_masked_write(chip,
			...
			FORCE_HVDCP_2p0, 0);
	if (rc < 0) {
		SMB_ERR(chip, "Couldn't force QC2.0 rc=%d\n", rc);
		return rc;
	}
...
static int set_usb_psy_dp_dm(struct smbchg_chip *chip, int state)
{
	...
	if (!rc && !(reg & USBIN_UV_BIT) && !(reg & USBIN_SRC_DET_BIT)) {
		...
		if (rc < 0) {
			SMB_ERR(chip, "Couldn't enable DP/DM for pulsing rc=%d\n", rc);
			return rc;
		}
	}
...
static void restore_from_hvdcp_detection(struct smbchg_chip *chip)
{
	...
	rc = smbchg_sec_masked_write(chip, chip->usb_chgpth_base + CHGPTH_CFG,
				HVDCP_ADAPTER_SEL_MASK, HVDCP_9V);
	if (rc < 0)
		SMB_ERR(chip, "Couldn't configure HVDCP 9V rc=%d\n", rc);
	...
	rc = smbchg_sec_masked_write(chip, chip->usb_chgpth_base + USB_AICL_CFG,
			AICL_EN_BIT, AICL_EN_BIT);
	if (rc < 0)
		SMB_ERR(chip, "Couldn't enable AICL rc=%d\n", rc);
	...
	if ((chip->schg_version == QPNP_SCHG_LITE)
		...
		SMB_DBG(chip, "Forcing 9V HVDCP 2.0\n");
		rc = force_9v_hvdcp(chip);
		if (rc)
			SMB_ERR(chip, "Failed to force 9V HVDCP=%d\n",	rc);
	}

	SMB_DBG(chip, "Retracting HVDCP vote for ICL\n");
	rc = vote(chip->usb_icl_votable, HVDCP_ICL_VOTER, false, 0);
	if (rc < 0)
		SMB_ERR(chip, "Couldn't retract HVDCP ICL vote rc=%d\n", rc);
}
...
static int smbchg_restricted_charging(struct smbchg_chip *chip, bool enable)
{
	...
	SMB_DBG(chip, "restricted_charging set to %d\n", enable);
...
static void handle_usb_removal(struct smbchg_chip *chip)
{
	...
	SMB_DBG(chip, "setting usb psy health UNKNOWN\n");
...
static void handle_usb_insertion(struct smbchg_chip *chip)
{
	...
	if (!chip->usb_ov_det) {
		 ...
		 */
		SMB_DBG(chip, "setting usb psy health %s\n",
...
static int otg_oc_reset(struct smbchg_chip *chip)
{
	...
	rc = smbchg_masked_write(chip, chip->bat_if_base + CMD_CHG_REG,
						OTG_EN_BIT, 0);
	if (rc)
		SMB_ERR(chip, "Failed to disable OTG rc=%d\n", rc);
	...
	rc = smbchg_masked_write(chip, chip->bat_if_base + CMD_CHG_REG,
						OTG_EN_BIT, OTG_EN_BIT);
	if (rc)
		SMB_ERR(chip, "Failed to re-enable OTG rc=%d\n", rc);
...
static void increment_aicl_count(struct smbchg_chip *chip)
{
	...
	SMB_DBG(chip, "aicl count c:%d dgltch:%d first:%ld\n",
	...
	if (chip->aicl_deglitch_short || chip->force_aicl_rerun) {
		...
		if (chip->aicl_irq_count > max_aicl_count) {
			SMB_DBG(chip, "elp:%ld first:%ld now:%ld c=%d\n",
				elapsed_seconds, chip->first_aicl_seconds,
				now_seconds, chip->aicl_irq_count);
			SMB_DBG(chip, "Disable AICL rerun\n");
			...
			if (rc < 0) {
				SMB_ERR(chip, "Couldn't disable hw aicl rerun rc=%d\n",
					rc);
				return;
			}
			...
			rc = vote(chip->usb_icl_votable, WEAK_CHARGER_ICL_VOTER,
					true, CURRENT_100_MA);
			if (rc < 0) {
				SMB_ERR(chip, "Can't vote %d current limit rc=%d\n",
					CURRENT_100_MA, rc);
			}
...
static int wait_for_usbin_uv(struct smbchg_chip *chip, bool high)
{
	...
	SMB_ERR(chip, "usbin uv didnt go to a %s state, still at %s, tries = %d, rc = %d\n",
			high ? "risen" : "lowered",
			usbin_uv ? "high" : "low",
			tries, rc);
	return -EINVAL;
...
static int wait_for_src_detect(struct smbchg_chip *chip, bool high)
{
	...
	SMB_ERR(chip, "src detect didn't go to a %s state, still at %s, tries = %d, rc = %d\n",
			high ? "risen" : "lowered",
			src_detect ? "high" : "low",
			tries, rc);
	return -EINVAL;
...
static int fake_insertion_removal(struct smbchg_chip *chip, bool insertion)
{
	...
	if (usbin_uv != insertion) {
		SMB_ERR(chip, "Skip faking, usbin uv is already %d\n", usbin_uv);
		return -EINVAL;
	}
	...
	if (src_detect == insertion) {
		SMB_ERR(chip, "Skip faking, src detect is already %d\n", src_detect);
		return -EINVAL;
	}

	SMB_DBG(chip, "Allow only %s charger\n",
	...
	SMB_DBG(chip, "Waiting on %s usbin uv\n",
			insertion ? "falling" : "rising");
	rc = wait_for_usbin_uv(chip, !insertion);
	if (rc < 0) {
		SMB_ERR(chip, "wait for usbin uv failed rc = %d\n", rc);
		return rc;
	}

	SMB_DBG(chip, "Waiting on %s src det\n",
			insertion ? "rising" : "falling");
	rc = wait_for_src_detect(chip, insertion);
	if (rc < 0) {
		SMB_ERR(chip, "wait for src detect failed rc = %d\n", rc);
		return rc;
	}
...
static int smbchg_prepare_for_pulsing(struct smbchg_chip *chip)
{
	...
	/* switch to 5V HVDCP */
	SMB_DBG(chip, "Switch to 5V HVDCP\n");
	rc = smbchg_sec_masked_write(chip, chip->usb_chgpth_base + CHGPTH_CFG,
				HVDCP_ADAPTER_SEL_MASK, HVDCP_5V);
	if (rc < 0) {
		SMB_ERR(chip, "Couldn't configure HVDCP 5V rc=%d\n", rc);
		goto out;
	}
	 ...
	 */
	if (!is_src_detect_high(chip)) {
		SMB_DBG(chip, "src det low after 500mS sleep\n");
		goto out;
	}
	...
	SMB_DBG(chip, "HVDCP voting for 300mA ICL\n");
	rc = vote(chip->usb_icl_votable, HVDCP_ICL_VOTER, true, 300);
	if (rc < 0) {
		SMB_ERR(chip, "Couldn't vote for 300mA HVDCP ICL rc=%d\n", rc);
		goto out;
	}
	...
	/* fake a removal */
	SMB_DBG(chip, "Faking Removal\n");
	rc = fake_insertion_removal(chip, false);
	if (rc < 0) {
		SMB_ERR(chip, "Couldn't fake removal HVDCP Removed rc=%d\n", rc);
		goto handle_removal;
	}
	...
	/* fake an insertion */
	SMB_DBG(chip, "Faking Insertion\n");
	rc = fake_insertion_removal(chip, true);
	if (rc < 0) {
		SMB_ERR(chip, "Couldn't fake insertion rc=%d\n", rc);
		goto handle_removal;
	}
	chip->hvdcp_3_det_ignore_uv = false;
	 ...
	 */
	if (!is_src_detect_high(chip)) {
		SMB_DBG(chip, "src det low after 2s sleep\n");
		rc = -EINVAL;
		goto out;
	}
	...
	if ((reg >> TYPE_BITS_OFFSET) != 0) {
		SMB_DBG(chip, "type bits set after 2s sleep - abort\n");
		rc = -EINVAL;
		goto out;
	}
...
static int smbchg_unprepare_for_pulsing(struct smbchg_chip *chip)
{
	...
	/* switch to 9V HVDCP */
	SMB_DBG(chip, "Switch to 9V HVDCP\n");
	rc = smbchg_sec_masked_write(chip, chip->usb_chgpth_base + CHGPTH_CFG,
				HVDCP_ADAPTER_SEL_MASK, HVDCP_9V);
	if (rc < 0) {
		SMB_ERR(chip, "Couldn't configure HVDCP 9V rc=%d\n", rc);
	...
	rc = smbchg_sec_masked_write(chip, chip->usb_chgpth_base + USB_AICL_CFG,
			AICL_EN_BIT, 0);
	if (rc < 0) {
		SMB_ERR(chip, "Couldn't disable AICL rc=%d\n", rc);
		return rc;
	}
	...
	rc = fake_insertion_removal(chip, false);
	if (rc < 0) {
		SMB_ERR(chip, "Couldn't fake removal rc=%d\n", rc);
		goto out;
	}
...
out:
	...
	if (rc < 0)
		SMB_ERR(chip, "Couldn't reduce aicl deglitch rc=%d\n", rc);
...
static int rerun_apsd(struct smbchg_chip *chip)
{
	...
	if (chip->schg_version == QPNP_SCHG_LITE) {
		...
		if (rc) {
			SMB_ERR(chip, "Couldn't re-run APSD rc=%d\n", rc);
			goto out;
		}

		SMB_DBG(chip, "Waiting on rising usbin uv\n");
		...
		SMB_DBG(chip, "Waiting on falling src det\n");
		...
		SMB_DBG(chip, "Waiting on falling usbin uv\n");
		...
		SMB_DBG(chip, "Waiting on rising src det\n");
...
static int smbchg_prepare_for_pulsing_lite(struct smbchg_chip *chip)
{
	...
	if (is_hvdcp_5v_cont_mode(chip)) {
		SMB_DBG(chip, "HVDCP by default is in 5V continuous mode\n");
		return 0;
	}
	...
	rc = rerun_apsd(chip);
	if (rc) {
		SMB_ERR(chip, "APSD rerun failed\n");
		goto out;
	}
	...
	if (!is_hvdcp_5v_cont_mode(chip)) {
		SMB_ERR(chip, "HVDCP could not be set in 5V continuous mode\n");
		goto out;
	}
...
out:
	...
	restore_from_hvdcp_detection(chip);
	if (!is_src_detect_high(chip)) {
		SMB_DBG(chip, "HVDCP removed - force removal\n");
...
static int smbchg_dp_dm(struct smbchg_chip *chip, int val)
{
	...
	case POWER_SUPPLY_DP_DM_PREPARE:
		if (!is_hvdcp_present(chip)) {
			SMB_ERR(chip, "No pulsing unless HVDCP\n");
			return -ENODEV;
		}
		if (chip->schg_version == QPNP_SCHG_LITE)
...
static int smbchg_get_prop_batt_charge_counter(struct smbchg_chip *chip)
{
	...
	if (rc < 0) {
		SMB_DBG(chip, "Couldn't get charge count rc = %d\n", rc);
		return rc;
	}
...
static int smbchg_get_prop_batt_current_max(struct smbchg_chip *chip)
{
	...
	if (rc < 0) {
		SMB_DBG(chip, "Couldn't get current max rc = %d\n", rc);
		return rc;
	}
...
{
	SMB_ERR(chip, "typec capability = %dma\n", val->intval);

	SMB_DBG(chip,"changing ICL from %dma to %dma\n", chip->typec_current_ma,
			val->intval);
...
static int smbchg_set_sdp_current(struct smbchg_chip *chip, int current_ma)
{
	if (chip->usb_supply_type == POWER_SUPPLY_TYPE_USB) {
		...
		SMB_DBG(chip, "from USB current_ma = %d\n", current_ma);
				       ...
				       const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_FLASH_ACTIVE:
		...
		if (rc) {
			SMB_ERR(chip, "Couldn't switch buck frequency, rc=%d\n", rc);
			/*
...
static irqreturn_t usbin_ov_handler(int irq, void *_chip)
{
	...
	if (reg & USBIN_OV_BIT) {
		...
		SMB_DBG(chip, "setting usb psy health OV\n");
...
static irqreturn_t usbin_uv_handler(int irq, void *_chip)
{
	...
	rc = smbchg_read(chip, &reg, chip->usb_chgpth_base + RT_STS, 1);
	if (rc) {
		SMB_ERR(chip, "could not read rt sts: %d", rc);
		goto out;
	}
	...
	if (!(reg & USBIN_UV_BIT) && !(reg & USBIN_SRC_DET_BIT)) {
		SMB_DBG(chip, "setting usb dp=f dm=f\n");
		rc = smbchg_request_dpdm(chip, true);
		if (rc < 0) {
			SMB_ERR(chip, "Couldn't enable DP/DM for pulsing rc=%d\n", rc);
			return rc;
	...
	if ((reg & USBIN_UV_BIT) && (reg & USBIN_SRC_DET_BIT) &&
		!chip->hvdcp_3_det_ignore_uv) {
		SMB_DBG(chip, "Very weak charger detected\n");
		chip->very_weak_charger = true;
		rc = smbchg_read(chip, &reg,
				chip->usb_chgpth_base + ICL_STS_2_REG, 1);
		if (rc) {
		...
		if ((reg & ICL_MODE_MASK) != ICL_MODE_HIGH_CURRENT) {
			...
			if (rc < 0)
				SMB_ERR(chip, "could not disable charger: %d", rc);
		...
		SMB_DBG(chip, "setting usb psy health UNSPEC_FAILURE\n");
		chip->usb_health = POWER_SUPPLY_HEALTH_UNSPEC_FAILURE;
		power_supply_changed(chip->usb_psy);
		schedule_work(&chip->usb_set_online_work);
	}
...
static irqreturn_t src_detect_handler(int irq, void *_chip)
{
	...
	if (rc < 0)
		SMB_ERR(chip, "Couldn't enable hw aicl rerun rc=%d\n", rc);

	rc = vote(chip->usb_suspend_votable, WEAK_CHARGER_EN_VOTER, false, 0);
	if (rc < 0)
		SMB_ERR(chip, "could not enable charger: %d\n", rc);
...
static irqreturn_t otg_oc_handler(int irq, void *_chip)
{
	...
	if (chip->schg_version == QPNP_SCHG_LITE) {
		SMB_WARN(chip, "OTG OC triggered - OTG disabled\n");
		return IRQ_HANDLED;
	}
	...
	if (chip->otg_retries < NUM_OTG_RETRIES) {
		...
		if (rc)
			SMB_ERR(chip, "Failed to reset OTG OC state rc=%d\n", rc);
		chip->otg_enable_time = ktime_get();
	}
	return IRQ_HANDLED;
}
...
static irqreturn_t usbid_change_handler(int irq, void *_chip)
{
	...
	SMB_DBG(chip, "setting usb psy OTG = %d\n",
			otg_present ? 1 : 0);
...
static int smbchg_hw_init(struct smbchg_chip *chip)
{
	...
	rc = smbchg_sec_masked_write(chip, chip->misc_base + TRIM_OPTIONS_7_0,
			...
			INPUT_MISSING_POLLER_EN_BIT);
	if (rc < 0) {
		SMB_ERR(chip, "Couldn't enable input missing poller rc=%d\n",
				rc);
		return rc;
	}
	...
	rc = smbchg_charging_en(chip, true);
	if (rc < 0) {
		SMB_ERR(chip, "Couldn't enable battery charging=%d\n", rc);
		return rc;
	}
	...
	if (chip->iterm_ma != -EINVAL) {
		if (chip->iterm_disabled) {
			SMB_ERR(chip, "Error: Both iterm_disabled and iterm_ma set\n");
			return -EINVAL;
		}
	...
	if (chip->resume_delta_mv != -EINVAL) {
		...
		if (!chip->chg_inhibit_source_fg) {
			...
			if (rc < 0) {
				SMB_ERR(chip, "Couldn't set inhibit val rc = %d\n",
						rc);
				return rc;
			}
		}
	...
	if (chip->wipower_dyn_icl_avail) {
		...
		if (rc < 0) {
			SMB_ERR(chip, "Couldn't set default wipower ilim = %d\n",
				rc);
			return rc;
		}
	}
	...
	if (rc < 0) {
		SMB_ERR(chip, "Couldn't unsuspend dc path= %d\n", rc);
		return rc;
	}
	...
	if (chip->force_aicl_rerun) {
		...
		if (rc < 0) {
			SMB_ERR(chip, "Couldn't vote enable hw aicl rerun rc=%d\n",
				rc);
			return rc;
		}
	}
		...
		struct ilim_map *map, char *property)
{
	...
	if (total_elements % RANGE_ENTRY) {
		SMB_ERR(chip, "%s table not in multiple of %d, total elements = %d\n",
				...
				int flags)
{
	...
	*irq_num = of_irq_get_byname(child, irq_name);
	if (*irq_num < 0) {
		SMB_ERR(chip, "Unable to get %s irqn", irq_name);
		rc = -ENXIO;
	}
	rc = devm_request_threaded_irq(chip->dev,
			*irq_num, NULL, irq_handler, flags, irq_name,
			chip);
	if (rc < 0) {
		SMB_ERR(chip, "Unable to request %s irq: %dn",
				irq_name, rc);
		rc = -ENXIO;
	}
	return 0;
...
static int smbchg_check_chg_version(struct smbchg_chip *chip)
{
	...
	revid_dev_node = of_parse_phandle(chip->pdev->dev.of_node,
					"qcom,pmic-revid", 0);
	if (!revid_dev_node) {
		SMB_ERR(chip, "Missing qcom,pmic-revid property - driver failed\n");
		return -EINVAL;
	}
	...
	case PMI8996:
		...
		break;
	default:
		SMB_ERR(chip, "PMIC subtype %d not supported, WA flags not set\n",
				pmic_rev_id->pmic_subtype);
	}

	SMB_DBG(chip, "pmic=%s, wa_flags=0x%x, hvdcp_supported=%s\n",
...
static void rerun_hvdcp_det_if_necessary(struct smbchg_chip *chip)
{
	...
	if (usb_supply_type == POWER_SUPPLY_TYPE_USB_DCP
		&& !is_hvdcp_present(chip)) {
		SMB_DBG(chip, "DCP found rerunning APSD\n");
		rc = vote(chip->usb_icl_votable,
				CHG_SUSPEND_WORKAROUND_ICL_VOTER, true, 300);
		if (rc < 0)
			SMB_ERR(chip, "Couldn't vote for 300mA for suspend wa, going ahead rc=%d\n",
					rc);

		rc = rerun_apsd(chip);
		if (rc)
			SMB_ERR(chip, "APSD rerun failed rc=%d\n", rc);

		read_usb_type(chip, &usb_type_name, &usb_supply_type);
		if (usb_supply_type != POWER_SUPPLY_TYPE_USB_DCP) {
			msleep(500);
			SMB_DBG(chip, "Rerun APSD as type !=DCP\n");
		...
		if (rc < 0)
			SMB_ERR(chip, "Couldn't vote for 0 for suspend wa, going ahead rc=%d\n",
					rc);
...
static int smbchg_probe(struct platform_device *pdev)
{
	...
	if (of_property_read_bool(pdev->dev.of_node, "qcom,external-typec")) {
		...
		if (rc) {
			SMB_ERR(chip, "failed to get prop typec-psy-name rc=%d\n",
				rc);
			return rc;
		}
	...
	if (IS_ERR(chip->extcon)) {
		SMB_ERR(chip, "failed to allocate extcon device\n");
	...
	if (rc) {
		SMB_ERR(chip, "failed to register extcon device\n");
		goto votables_cleanup;
	}
...
static void smbchg_shutdown(struct platform_device *pdev)
{
	...
	SMB_DBG(chip, "Reducing to 500mA\n");
	rc = vote(chip->usb_icl_votable, SHUTDOWN_WORKAROUND_ICL_VOTER, true,
			500);
	if (rc < 0)
		SMB_ERR(chip, "Couldn't vote 500mA ICL\n");

	SMB_DBG(chip, "Disable Parallel\n");
	mutex_lock(&chip->parallel.lock);
	smbchg_parallel_en = 0;
	smbchg_parallel_usb_disable(chip);
	mutex_unlock(&chip->parallel.lock);

	SMB_DBG(chip, "Disable all interrupts\n");
	...
	if (rc < 0)
		SMB_ERR(chip, "Couldn't vote to enable indirect AICL rerun\n");
	rc = vote(chip->hw_aicl_rerun_disable_votable,
		WEAK_CHARGER_HW_AICL_VOTER, false, 0);
	if (rc < 0)
		SMB_ERR(chip, "Couldn't vote to enable AICL rerun\n");
	...
	SMB_DBG(chip, "Wait 500mS to lower to 5V\n");
	/* wait for HVDCP to lower to 5V */
	msleep(500);
	/*
	 * Check if the same hvdcp session is in progress. src_det should be
	...
	SMB_DBG(chip, "Wait 1S to settle\n");
	msleep(1000);
	chip->hvdcp_3_det_ignore_uv = false;

	SMB_DBG(chip, "wrote power off configurations\n");
}
